...
from qwen2_5_omni.utils import GenerationMixin
...
from qwen2_5_omni.sdpa_attention import sdpa_attention_forward
from qwen2_5_omni.flash_attention import flash_attention_forward
import transformers.modeling_utils as modeling_utils
ALL_ATTENTION_FUNCTIONS["sdpa_bias"] = sdpa_attention_forward
...
class Qwen2_5OmniAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if bias is not None or bias_matrix is not None:
            attention_interface = ALL_ATTENTION_FUNCTIONS["sdpa_bias"]
...
class Qwen2_5OmniDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Qwen2_5OmniTextConfig, layer_idx: int, dynamic_fac=False, gated_fc=False):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.gated_fc = gated_fc
        if gated_fc:
            self.focus_biasfc = nn.Linear(config.hidden_size, 1)
            self.supress_biasfc = nn.Linear(config.hidden_size, 1)
            self.instruct_biasfc = nn.Linear(config.hidden_size, 1)
            # self.bias_scale = nn.Parameter(torch.tensor(1.0))
            self.focus_biasfac = None
        else:
            if dynamic_fac:
                self.focus_biasfac = nn.Parameter(torch.zeros(1))
                self.supress_biasfac = nn.Parameter(torch.zeros(1))
                self.instruct_biasfac = nn.Parameter(torch.zeros(1))
                self.bias_scale = nn.Parameter(torch.tensor(1.0))
            else:
                self.focus_biasfac = None
                self.supress_biasfac = None
                self.instruct_biasfac = None
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        Args:
            ...
            kwargs (`dict`, *optional*):
                ...
                into the model
        """
        hidden_states = self._move_to_layer_device(hidden_states)
        attention_mask = self._move_to_layer_device(attention_mask)
        position_ids = self._move_to_layer_device(position_ids)
        past_key_values = self._move_to_layer_device(past_key_values)
        cache_position = self._move_to_layer_device(cache_position)
        position_embeddings = self._move_to_layer_device(position_embeddings)
        bias = self._move_to_layer_device(bias)
        ...
        if self.gated_fc:
            ...
            ins = bias[..., 0]      # instruct 区域 mask
            pos = bias[..., 1]      # focus 区域 mask
            neg = bias[..., 2]      # suppress 区域 mask
            base = bias[..., 3]
            
            instruct_out = self.instruct_biasfc(hidden_states).permute(0, 2, 1)
            focus_out    = self.focus_biasfc(hidden_states).permute(0, 2, 1)
            supress_out  = self.supress_biasfc(hidden_states).permute(0, 2, 1)
            
            raw_ins = instruct_out.expand(B, N, N).unsqueeze(1)
            raw_pos =  focus_out.expand(B, N, N).unsqueeze(1)
            raw_neg =  supress_out.expand(B, N, N).unsqueeze(1)
            
            ins_out  = torch.sigmoid(raw_ins) * 2          # → 0~2
            pos_out  = 1 + torch.sigmoid(raw_pos)          # → 1~2
            neg_out  = torch.sigmoid(raw_neg)              # → 0~1
            base_out = torch.ones_like(base)               # → 1
            bias = (
                ins_out * ins +
                pos_out * pos +
                neg_out * neg +
            ...
            if (bias is not None) and (self.focus_biasfac is not None) and (self.supress_biasfac is not None) and (self.instruct_biasfac is not None):
                # bias = self.instruct_biasfac*bias[...,0] + self.focus_biasfac*bias[...,1] + self.supress_biasfac*bias[...,2] + bias[...,3]
                ins = bias[..., 0]      # instruct 区域 mask
                pos = bias[..., 1]      # focus 区域 mask
                neg = bias[..., 2]      # suppress 区域 mask
                base = bias[..., 3]

                # baseline = 0，不使用 bias[...,3]
                # bias_matrix = (
                #     self.instruct_biasfac * ins +
                #     self.focus_biasfac    * pos +
                #     self.supress_biasfac  * neg
                # ) * self.bias_scale
                # raw_gate = (
                #     torch.abs(self.instruct_biasfac) * ins +
                #     torch.abs(self.focus_biasfac)    * pos +
                #     -torch.abs(self.supress_biasfac)  * neg
                # )
                # bias_matrix = (torch.tanh(raw_gate) + bias[..., 3]) * self.bias_scale
                # u = self.instruct_biasfac   # learnable scalar

                # focus_val    = torch.sigmoid(u)
                # supress_val  = -(1.0 - torch.sigmoid(u))
                # instruct_val = torch.tanh(u)
                # instruct_val = self.instruct_biasfac            # free sign
                # focus_val    = torch.abs(self.focus_biasfac)    # must enhance
                # supress_val  = -torch.abs(self.supress_biasfac) # must suppress
                focus_val    = F.softplus(self.focus_biasfac)     # > 0
                supress_val  = -F.softplus(self.supress_biasfac)  # < 0
                instruct_val = self.instruct_biasfac     # (-1,1)
                raw_gate = (
                    instruct_val * ins +
                    focus_val    * pos +
                    supress_val  * neg 
                )
                bias_matrix = torch.tanh(raw_gate) * self.bias_scale   # (B,T,T)
                bias = None
            else:
                bias_matrix = None
    ...
    def _move_to_layer_device(self, x):
        """
        Recursively move tensors in x to the same device as this layer.
        Supports Tensor / tuple / list / dict / nested structures.
        """
        if x is None:
            return None
        ...
        if torch.is_tensor(x):
            if x.device != layer_device:
                return x.to(layer_device)
            return x

        elif isinstance(x, tuple):
            return tuple(self._move_to_layer_device(v) for v in x)

        elif isinstance(x, list):
            return [self._move_to_layer_device(v) for v in x]

        elif isinstance(x, dict):
            return {k: self._move_to_layer_device(v) for k, v in x.items()}
...
class Qwen2_5OmniThinkerTextModel(Qwen2_5OmniPreTrainedModel):
    ...
    def __init__(self, config: Qwen2_5OmniTextConfig, dynamic_fac=False, gated_fc=False):
        super().__init__(config)
        ...
        self.layers = nn.ModuleList(
            [Qwen2_5OmniDecoderLayer(config, layer_idx, dynamic_fac=dynamic_fac, gated_fc=gated_fc) for layer_idx in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        for decoder_layer_idx, decoder_layer in enumerate(self.layers):
            ...
            layer_outputs, bias_matrix = decoder_layer(
                hidden_states,
            ...
            # ##########################
            # save_path = f"/projects_vol/gp_aseschng/wang.tianrui/codes/temp/gate_bias/bias_layer_{decoder_layer_idx}.pt"
            # # 当前层的 bias: [B, 1, m, L]
            # cur_bias = bias_matrix.detach().cpu()
            # # 1. 如果已有历史文件 → load 旧结果
            # if os.path.exists(save_path):
            #     old_bias = torch.load(save_path)   # old: [B, 1, M_old, L]
            #     # 2. 在 m 维拼接（dim=2）
            #     old_bias, cur_bias = pad_bias_to_same_length(old_bias, cur_bias)
            #     new_bias = torch.cat([old_bias, cur_bias], dim=2)
            # else:
            #     new_bias = cur_bias
            # # 3. 保存新累积的 bias
            # torch.save(new_bias, save_path)
            # ##########################
        ...
        ####################
        # visualize_all_layers(len(self.layers))
        ####################
    ...
    def _move_to_layer_device(self, x):
        ...
        layer_device = self.norm.weight.device
    ...
    @classmethod
    def _from_config(cls, config, dynamic_fac=False, gated_fc=False, **kwargs):
        """
        ...
        """
        kwargs["dynamic_fac"] = dynamic_fac
        kwargs["gated_fc"] = gated_fc
        ...
        if modeling_utils.is_deepspeed_zero3_enabled() and not modeling_utils._is_quantized and not modeling_utils._is_ds_init_called:
            logger.info("Detected DeepSpeed ZeRO-3: activating zero.init() for this model")
            # this immediately partitions the model across all gpus, to avoid the overhead in time
            # and memory copying it on CPU or each GPU first
            import deepspeed

            init_contexts = [deepspeed.zero.Init(config_dict_or_path=modeling_utils.deepspeed_config()), modeling_utils.set_zero3_state()]
            with modeling_utils.ContextManagers(init_contexts):
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        self.model = Qwen2_5OmniThinkerTextModel._from_config(config=config.text_config)
...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        thinker_result = self.thinker.generate(input_ids=input_ids, bias_config=bias_config, **thinker_kwargs)
...
class DynamicAttenQwen2_5omnithinker(Qwen2_5OmniThinkerForConditionalGeneration):
    config: Qwen2_5OmniThinkerConfig
    base_model_prefix = "thinker"
    _tied_weights_keys = ["model.embed_tokens.weight", "lm_head.weight"]
    _no_split_modules = ["Qwen2_5OmniAudioEncoder", "Qwen2_5OmniVisionEncoder"]
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        self.visual = Qwen2_5OmniVisionEncoder._from_config(config.vision_config)
        self.vocab_size = config.text_config.vocab_size
        self.model = Qwen2_5OmniThinkerTextModel._from_config(config=config.text_config, dynamic_fac=True)
        self.lm_head = nn.Linear(config.text_config.hidden_size, config.text_config.vocab_size, bias=False)
        self.pad_token_id = self.config.pad_token_id if self.config.pad_token_id is not None else -1
        self.spatial_merge_size = config.vision_config.spatial_merge_size
        self.rope_deltas = None
...
class GatedAttenQwen2_5omnithinker(Qwen2_5OmniThinkerForConditionalGeneration):
    config: Qwen2_5OmniThinkerConfig
    base_model_prefix = "thinker"
    _tied_weights_keys = ["model.embed_tokens.weight", "lm_head.weight"]
    _no_split_modules = ["Qwen2_5OmniAudioEncoder", "Qwen2_5OmniVisionEncoder"]
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        self.visual = Qwen2_5OmniVisionEncoder._from_config(config.vision_config)
        self.vocab_size = config.text_config.vocab_size
        self.model = Qwen2_5OmniThinkerTextModel._from_config(config=config.text_config, dynamic_fac=True, gated_fc=True)
        self.lm_head = nn.Linear(config.text_config.hidden_size, config.text_config.vocab_size, bias=False)
        self.pad_token_id = self.config.pad_token_id if self.config.pad_token_id is not None else -1
        self.spatial_merge_size = config.vision_config.spatial_merge_size
        self.rope_deltas = None
...
def plot_attention_bias_only(
    ...
    for b in range(B):
        ...
        save_path = os.path.join(save_dir, f"{name}_{b}.png")
        plt.savefig(save_path, dpi=dpi, bbox_inches="tight")
        plt.close()

        print(f"Saved: {save_path}")
    
def visualize_all_layers(num_layers):
    base = "/projects_vol/gp_aseschng/wang.tianrui/codes/temp/gate_bias"

    for layer_idx in range(num_layers):
        path = os.path.join(base, f"bias_layer_{layer_idx}.pt")
        if not os.path.exists(path):
            print(f"[WARN] Layer {layer_idx} no bias file found.")
            continue

        bias_all = torch.load(path)  # [B, 1, M_total, L]

        # 只画某个 batch，例如 b=0
        plot_attention_bias_only(
            bias_all,
            save_dir=base,
            name=f"layer_{layer_idx}_accum",
            dpi=150
        )

        print(f"绘制完 Layer {layer_idx} 累积 bias，共 {bias_all.shape[2]} 行")
        # os.remove(path)
        
def pad_bias_to_same_length(old_bias, cur_bias):
    """
    old_bias: [B, 1, M_old, L_old]
    cur_bias: [B, 1, m_new, L_new]
    目标：pad 到统一 L_max
    """

    L_old = old_bias.shape[-1]
    L_new = cur_bias.shape[-1]
    L_max = max(L_old, L_new)

    # pad 最右侧
    if L_old < L_max:
        pad_width = (0, L_max - L_old)
        old_bias = F.pad(old_bias, pad_width, value=0)

    if L_new < L_max:
        pad_width = (0, L_max - L_new)
        cur_bias = F.pad(cur_bias, pad_width, value=0)

    return old_bias, cur_bias