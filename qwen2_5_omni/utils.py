...
class GenerationMixin(ContinuousMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        #     model_kwargs["bias_config"] = kwargs["bias_config"]
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # history_input_ids.append(model_inputs["input_ids"])
            # print(f"model_inputs[input_ids]:{model_inputs['input_ids']}")
            if self.bias_config is not None:
                with torch.no_grad():
                    if self.model.layers[0].focus_biasfac is not None or self.model.layers[0].focus_biasfc is not None:
                        temp = generate_attention_bias_3strength(
                            # input_ids=torch.cat(history_input_ids, dim=-1),
                            input_ids=input_ids,
                            **self.bias_config
                        )
                        pos_mask = (temp == self.bias_config["focus_strength"]).float()
                        neg_mask = (temp == self.bias_config["suppress_strength"]).float()
                        ins_mask = (temp == self.bias_config["instruct_strength"]).float()
                        other_mask = 1.0 - pos_mask - neg_mask - ins_mask
                        model_inputs["bias"] = torch.stack([
                            ins_mask, pos_mask, neg_mask, other_mask
                        ], dim=-1).to(input_ids.device)
                    else:
                        model_inputs["bias"] = generate_stepwise_attention_bias(
                            input_ids=torch.cat(history_input_ids, dim=-1),
                            **self.bias_config
                        ).to(input_ids.device)
            ...
            if is_prefill:
                ...
                model_inputs["bias"] = model_inputs["bias"][:, :, -1:, :]
                # batch infer
                model_inputs["bias"] *= unfinished_sequences[:, None, None, None, None]
        ...
        # processor = Qwen2_5OmniProcessor.from_pretrained(r"/scratch/n2409728b/qa_lora_normscore_bal_qwen3cot_thinktoken_fullysave_abdyna_enzh/checkpoint_13000")
        # inspect_attention_bias_batch(
        #     examples={
        #         "input_ids": torch.cat(history_input_ids, dim=-1),
        #         "bias": temp
        #     },
...
def generate_stepwise_attention_bias(
    input_ids: torch.Tensor,
    audio_token_id: int = 151646,
    score_start_id: int = 151647,
    score_end_id: int = 151648,
    focus_strength: float = 2.0,
    suppress_strength: float = 0.3,
) -> torch.Tensor:
    """
    在推理时动态生成注意力 bias。
    每一步调用一次，根据当前上下文判断是否处于 <score>...</score> 区间。
    
    - 预测 <score> 时不加 bias（下一 token 开始启用）
    - 处于 <score>...</score> 区间时启用 bias
    - 预测 </score> 时 bias 仍启用（最后一 token）
    - 之后不再启用 bias
    
    返回:
        bias: [1, 1, L, L]
    """
    assert input_ids.dim() == 2 and input_ids.size(0) == 1, "仅支持 batch=1 推理"
    ids = input_ids[0].tolist()
    L = len(ids)

    # === Step 1️⃣ 初始化 ===
    bias = torch.ones((1, 1, L, L), device=input_ids.device, dtype=torch.float32)

    # === Step 2️⃣ 计算当前上下文中是否处于 <score> 区间 ===
    start_positions = [i for i, x in enumerate(ids) if x == score_start_id]
    end_positions   = [i for i, x in enumerate(ids) if x == score_end_id]
    
    # 判断“当前正在生成的位置”（即当前最后一个 token）是否在激活区间
    active = False
    if start_positions:
        last_start = start_positions[-1]
        # 找到离它最近的结束符
        next_end = next((x for x in end_positions if x > last_start), None)
        # 若还未遇到 </score>，说明激活中（但不包含刚生成 <score> 自身）
        if next_end is None and (L - 1) > last_start:
            active = True
        # 若已经在 </score> 内部（包括它本身）
        elif next_end is not None and (L - 1) <= next_end and (L - 1) > last_start:
            active = True

    # === Step 3️⃣ 若未激活，直接返回全 1 ===
    if not active:
        return bias

    # === Step 4️⃣ 标记 audio token ===
    audio_mask = (input_ids == audio_token_id).unsqueeze(1).unsqueeze(2)  # [1,1,1,L]

    # === Step 5️⃣ 当前 query 只针对最后一个位置 ===
    score_mask = torch.zeros((1, 1, L, 1), device=input_ids.device, dtype=torch.bool)
    score_mask[:, :, -1:, :] = True  # 仅当前 query 位置生效

    # === Step 6️⃣ 应用 bias ===
    bias = torch.where(
        score_mask & audio_mask,
        torch.tensor(focus_strength, device=input_ids.device),
        bias
    )
    bias = torch.where(
        score_mask & (~audio_mask),
        torch.tensor(suppress_strength, device=input_ids.device),
        bias
    )
    return bias


def generate_stepwise_attention_bias_3strength(
    input_ids: torch.Tensor,
    audio_token_id: int = 151646,
    score_start_id: int = 151647,
    score_end_id: int = 151648,
    focus_audio_start_id: int = -9999,
    focus_audio_end_id: int = -9999,
    focus_strength: float = 2.0,
    suppress_strength: float = 0.3,
    instruct_strength: float = 0.1,
):
    """
    Step-wise 推理专用的 3-strength 注意力 bias 生成器。
    - 支持 <score>...</score>
    - 支持 <focus_audio>...</focus_audio>
    - 每一步只对最后一个 query（当前生成位置）施加 bias
    - 逻辑与 batch 版本保持一致
    """
    ...
    def check_active(start_positions, end_positions):
        """
        判断 stepwise 当前最后一个 token 是否处于指定 span 内部。
        """
        ...
        # 已闭合区间： <start> ... </end>
        if next_end is not None and last_start < (L - 1) <= next_end:
            return True

        return False

    score_active = check_active(start_positions, end_positions)

    # === 2️⃣ focus_audio span 区间 ===
    fa_start_positions = [i for i, x in enumerate(ids) if x == focus_audio_start_id]
    fa_end_positions   = [i for i, x in enumerate(ids) if x == focus_audio_end_id]
    focus_audio_active = check_active(fa_start_positions, fa_end_positions)

    # === 3️⃣ unified span：任意一个激活即可 ===
    active = score_active or focus_audio_active

    # === 4️⃣ 不在任何 span → 返回全 1 ===
    if not active:
        return bias

    # === 5️⃣ audio token mask ===
    audio_mask = (input_ids == audio_token_id).unsqueeze(1).unsqueeze(2)  # [1,1,1,L]

    # === 6️⃣ 当前 query mask：只有最后一个位置生效 ===
    q_mask = torch.zeros((1, 1, L, 1), device=input_ids.device, dtype=torch.bool)
    q_mask[:, :, -1:, :] = True

    # === 7️⃣ 应用 focus / suppress ===
    bias = torch.where(
        q_mask & audio_mask,
        torch.tensor(focus_strength, device=input_ids.device),
        bias,
    ...
    # === 8️⃣ 应用 instruct（对 span 内 query → 限制 attention 到 audio 之前部分）===
    audio_positions = (input_ids[0] == audio_token_id).nonzero(as_tuple=True)[0]
    if len(audio_positions) > 0:
        first_audio = audio_positions[0].item()

        # 当前 query = 最后一个
        q = L - 1
        if first_audio > 0:
            bias[0, 0, q, :first_audio] = instruct_strength

    return bias


def generate_attention_bias_3strength(
    input_ids: torch.Tensor,
    audio_token_id: int = 151646,
    score_start_id: int = 151647,
    score_end_id: int = 151648,
    ...
    """
    Autoregressive-safe attention bias with correct span semantics:
    - start token itself is NOT inside the span.
    - span begins from the next token.
    - if end token missing (common in inference), span continues to sequence end.
    """

    B, L = input_ids.shape
    device = input_ids.device

    bias = torch.ones((B, 1, L, L), device=device, dtype=torch.float32) * -98
    audio_mask = (input_ids == audio_token_id).unsqueeze(1).unsqueeze(2)

    score_ranges = torch.zeros((B, L), dtype=torch.bool, device=device)
    focus_audio_ranges = torch.zeros((B, L), dtype=torch.bool, device=device)
    ...
    for b in range(B):
        ...
        for i, tok in enumerate(ids):
            if tok == score_start_id:
                score_on = True
                continue  # 🚫 start 不进入 span

            if score_on:
                score_ranges[b, i] = True

            if tok == score_end_id:
                # 包含 end 位置（保持你的 e+1 行为）
                score_ranges[b, i] = True
                score_on = False
        ...
        focus_on = False
        for i, tok in enumerate(ids):
            if tok == focus_audio_start_id:
                focus_on = True
                continue  # 🚫 start 不进入 span

            if focus_on:
                focus_audio_ranges[b, i] = True

            if tok == focus_audio_end_id:
                focus_audio_ranges[b, i] = True
                focus_on = False

    # 合并 span
    unified_ranges = (score_ranges | focus_audio_ranges)
    unified_mask = unified_ranges.unsqueeze(1).unsqueeze(3)

    # focus / suppress bias
    bias = torch.where(unified_mask & audio_mask,
                       torch.tensor(focus_strength, device=device),
                       bias)
    bias = torch.where(unified_mask & (~audio_mask),
                       torch.tensor(suppress_strength, device=device),
                       bias)
    ...
    for b in range(B):
        ...
        if len(aud_pos) == 0:
            continue
        first_audio = aud_pos[0].item()

        for q in range(L):
            if unified_ranges[b, q]:
                bias[b, 0, q, :first_audio] = instruct_strength
...
# Visualisation utilities below (inspect_attention_bias_batch) are only used
# for debugging / paper figures. Keep them optional so a pure-inference
# environment does not need matplotlib/seaborn.
try:
    import matplotlib
...
def inspect_attention_bias_batch(
    examples,
    tokenizer,
    audio_token_id=151646,
    save_dir="/projects_vol/gp_aseschng/wang.tianrui/codes/temp/",
    font_family="SimHei",  # 中文字体（可改成 "Noto Sans CJK JP"）
    dpi=400,               # 超高分辨率
    token_fontsize=1,      # token 字体大小
    title_fontsize=10,     # 标题字体大小
):
    """
    高分辨率可读版 attention bias 检查：
      ✅ 解决中文乱码（可选字体）
      ✅ 自动缩放字体避免重叠
      ✅ 提高分辨率 (DPI=800)
      ✅ 标注 Audio/Score 区域
    ...
    if not _HAS_VIZ:
        raise ImportError(
            "inspect_attention_bias_batch() requires matplotlib and seaborn; "
            "install them if you want to generate these debug figures."
        )
    ...
    with open(log_path, "a", encoding="utf-8") as log_f:
        ...
        input_ids = examples["input_ids"]  # [B, L]
        bias = examples["bias"]            # [B, 1, L, L]
        B, _, Lq, Lk = bias.shape

        score_start_id = tokenizer.convert_tokens_to_ids("<score>")
        score_end_id   = tokenizer.convert_tokens_to_ids("</score>")

        print(f"🔍 检查 {B} 个样本，每个序列长度 = {Lq}")

        for b in range(B):
            ids = input_ids[b]
            cur_bias = bias[b, 0]

            # === Step 1️⃣ 构造 token 标签 ===
            tokens = [tokenizer.decode([tid.item()], skip_special_tokens=False) for tid in ids]
            short_labels = [(t if len(t) <= 6 else t[:6] + "…") for t in tokens]

            # === Step 2️⃣ 标识区域 ===
            score_mask = (ids == score_start_id) | (ids == score_end_id)
            score_positions = torch.nonzero(score_mask).flatten().tolist()
            audio_positions = torch.nonzero(ids == audio_token_id).flatten().tolist()
            audio_end = max(audio_positions) if len(audio_positions) > 0 else -1

            print(f"\n=== [Sample {b}] ===")
            print(f"<score> 位置: {score_positions}")
            print(f"<|AUDIO|> 区间: 0 ~ {audio_end}")
            ...
            # 小字体，防止重叠
            plt.xticks(rotation=90, fontsize=token_fontsize)
            plt.yticks(rotation=0, fontsize=token_fontsize)

            # 区域线
            for pos in score_positions:
                ax.axhline(pos, color="lime", linestyle="--", linewidth=0.5)
                ax.axvline(pos, color="lime", linestyle="--", linewidth=0.5)

            if audio_end > 0:
                ax.axhline(audio_end, color="cyan", linestyle="--", linewidth=0.5)
                ax.axvline(audio_end, color="cyan", linestyle="--", linewidth=0.5)
                ax.text(
                    audio_end / 2, -3, "Audio Region",
                    color="blue", fontsize=6, ha="center", va="top"
                )

            plt.title(
                f"Sample {b} — Attention Bias (Audio=Blue, Score=Green)",
                fontsize=title_fontsize
            )

            save_path = os.path.join(save_dir, f"infer_sample_{b}_hires.png")
            plt.tight_layout()
            plt.savefig(save_path, dpi=dpi, bbox_inches="tight")
            plt.close()

            print(f"✅ 图像已保存: {os.path.abspath(save_path)}")
            print(f"min={cur_bias.min().item():.3f}, max={cur_bias.max().item():.3f}, mean={cur_bias.mean().item():.3f}")

        sys.stdout = sys.__stdout__
        print(f"📝 所有输出已写入: {os.path.abspath(log_path)}")